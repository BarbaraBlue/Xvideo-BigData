...
# Se busca en la pagina Xvideo: LOVE con filtro de valoracion

# Inicializando la var de archivo con el nombre de la página a utilizar
paginaXVideos <- 'https://www.xvideos.com/?k=LOVE&top'
...
Tipo <- list("8","8","8","8","8","8","8","8","8","","8","8","8","8","8","8","8","8","8","8","8","8","8","8","8","8","8")
unlistTipo <- unlist(Tipo)

# Se genera una tabla con los datos obtenidos para GAY
dfLOVE <- data.frame(LINKS = LinksXvideo, TITULO= textoXVideos, TIPO= unlistTipo, VISITAS= unlistVisitas, ME_GUSTA= unlistMe_Gusta, NO_ME_GUSTA= unlistNo_Me_Gusta)

# Almacenando la informacion en CSV
write.csv(dfLOVE, file="Tabla08.csv")