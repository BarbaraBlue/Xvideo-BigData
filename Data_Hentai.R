...
# Se busca en la pagina Xvideo: HENTAI con filtro de valoracion

# Inicializando la var de archivo con el nombre de la página a utilizar
paginaXVideos <- 'https://www.xvideos.com/?k=HENTAI&top'
...
# Se genera una variable tipo, donde 9 es HENTAI
Tipo <- list("9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9","9")
unlistTipo <- unlist(Tipo)

# Se genera una tabla con los datos obtenidos para HENTAI
dfHENTAI <- data.frame(LINKS = LinksXvideo,TITULO= textoXVideos, TIPO= unlistTipo, VISITAS= unlistVisitas, ME_GUSTA= unlistMe_Gusta, NO_ME_GUSTA= unlistNo_Me_Gusta)

ordereddfHENTAI <- arrange(dfHENTAI, VISITAS)

# Almacenando la informacion en CSV
write.csv(dfHENTAI, file="Tabla09.csv")

# Graficando VISITAS la tabla en horizontal
grafico_HENTAI <- ggplot(dfHENTAI, aes(TITULO, VISITAS)) +
  geom_bar(stat = "identity",fill = rgb(0.2, 0.2, 1, 0.3), color = "blue") +
  coord_flip() + 
  theme_minimal()

g_HENTAIlikes <- ggplot(dfHENTAI, aes(TITULO, ME_GUSTA)) +
  geom_bar(stat = "identity",fill = rgb(0.2, 0.2, 1, 0.3), color = "blue") +
  coord_flip() + 
  theme_minimal()

g_HENTAIdislikes <- ggplot(dfHENTAI, aes(TITULO, NO_ME_GUSTA)) +
  geom_bar(stat = "identity",fill = rgb(0.2, 0.2, 1, 0.3), color = "blue") +
  coord_flip() + 
  theme_minimal()
