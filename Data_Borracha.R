...
# # Se busca en la pagina Xvideo: BORRACHA DORMIDA

# Inicializando la var de archivo con el nombre de la página a utilizar
paginaXVideos <- 'https://www.xvideos.com/?k=borracha%20dormida&top'
...
# Se genera una variable tipo, donde 2 es BORRACHA DORMIDA
Tipo <- list("2","2","2","2","2","2","2","2","2","2","2","2","2","2","2","2","2","2","2","2","2","2","2","2","2","2","2")
unlistTipo <- unlist(Tipo)

# Se genera una tabla con los datos obtenidos para BORRACHA DORMIDA
dfBORRACHA <- data.frame(LINKS= LinksXvideo, TITULO= textoXVideos, TIPO= unlistTipo, VISITAS= unlistVisitas, ME_GUSTA= unlistMe_Gusta, NO_ME_GUSTA= unlistNo_Me_Gusta)

# Almacenando la informacion en CSV
write.csv(dfBORRACHA, file="Tabla02.csv")
