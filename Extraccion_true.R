...
# Funcion for que no logro hacer funcionar 
...
dfvideos <- data.frame(LINKS = LinksXvideo, TITULO = textoXVideos, DURACION = DuracionXVideos, VISITAS= unlistVisitas, ME_GUSTA= unlistMe_Gusta, NO_ME_GUSTA= unlistNo_Me_Gusta)

# Almacenando la informacion en CSV
write.csv(dfvideos, file="TablaXVideos01.csv")

# No puedo graficar, problema: no se encuentra la funcion %>% ???
dfvideos %>%
  ggplot() +
  aes(x = VISTAS, y = ME_GUSTA) +
