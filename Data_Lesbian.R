...
# # Se busca en la pagina Xvideo: LESBIAN con filtro de valorizacion

# Inicializando la var de archivo con el nombre de la página a utilizar
paginaXVideos <- 'https://www.xvideos.com/?k=lesbian&top'
...
# Se genera una variable tipo, donde 4 es LESBIAN
Tipo <- list("4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4","4")
unlistTipo <- unlist(Tipo)

# Se genera una tabla con los datos obtenidos para LESBIAN
dfLESBIAN<- data.frame(LINKS= LinksXvideo, TITULO= textoXVideos, TIPO= unlistTipo, VISITAS= unlistVisitas, ME_GUSTA= unlistMe_Gusta, NO_ME_GUSTA= unlistNo_Me_Gusta)

# Almacenando la informacion en CSV
write.csv(dfLESBIAN, file="Tabla04.csv")
