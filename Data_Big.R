...
# Se busca en la pagina Xvideo: BIG DICK con filtro de valoracion

# Inicializando la var de archivo con el nombre de la página a utilizar
paginaXVideos <- 'https://www.xvideos.com/?k=big%20dicK&top'
...
# Se genera una variable tipo, donde 6 es BIG DICK
Tipo <- list("6","6","6","6","6","6","6","6","6","6","6","6","6","6","6","6","6","6","6","6","6","6","6","6","6","6","6")
unlistTipo <- unlist(Tipo)

# Se genera una tabla con los datos obtenidos para GAY
dfBIG <- data.frame(LINKS = LinksXvideo, TITULO= textoXVideos, TIPO= unlistTipo, VISITAS= unlistVisitas, ME_GUSTA= unlistMe_Gusta, NO_ME_GUSTA= unlistNo_Me_Gusta)

# Almacenando la informacion en CSV
write.csv(dfBIG, file="Tabla06.csv")
